...
VIEW_ID / CONTENT_ID 在运行时随机生成，确保每次部署唯一。
"""
import json, sys, re, time, os, random, requests
...
def rand_id() -> str:
    """生成随机 DataEase 风格 ID（毫秒时间戳 + 随机后缀）"""
...
def main():
    ...
    # 运行时随机生成，覆盖 params 里的固定值
    view_id    = rand_id()
    content_id = rand_id()
    params["VIEW_ID"]    = view_id
    params["CONTENT_ID"] = content_id
    # SCENE_ID 用占位值（saveCanvas 时 payload.id=None，服务端自动分配真实 dashboard_id）
    params.setdefault("SCENE_ID", "0")
